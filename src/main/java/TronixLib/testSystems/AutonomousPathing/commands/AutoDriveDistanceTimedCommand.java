 ...
 *
 */
public class AutoDriveDistanceTimedCommand extends Command {
	...
	private static final double kI = 0.008; //0.018
	...
	public static final double MAX_OUTPOUT_SPEED = 0.7; // 0.6 test yesterday// 0.55 competition

	private double m_error = 0;
	double m_OUTPUT_SPEED_Value;
	//double m_SensValue;
	double m_time;
	...
	public AutoDriveDistanceTimedCommand(double distance, int direction, double time) {
		if(direction == 0){
	...
	protected void execute() {
		...
		SmartDashboard.putNumber("EncoderPIDSetPointCorrection",-m_OUTPUT_SPEED_Value);
		...
		Robot.driveTrain.arcadeDrive(m_OUTPUT_SPEED_Value, turningValue); // robot de test
	...
	private class MyPidOutput implements PIDOutput {
		...
		public void pidWrite(double output) {
			m_OUTPUT_SPEED_Value = output;
			if(m_OUTPUT_SPEED_Value > MAX_OUTPOUT_SPEED) //0.55 robot de competition
				m_OUTPUT_SPEED_Value = MAX_OUTPOUT_SPEED;
			else if(m_OUTPUT_SPEED_Value < -MAX_OUTPOUT_SPEED)
				m_OUTPUT_SPEED_Value = -MAX_OUTPOUT_SPEED;	
		}
	}
}
