...
public class Robot extends TimedRobot {
	...
	public void robotInit() {
		...
		_talon.configMotionCruiseVelocity(Constants.kCruiseVelocity, Constants.kTimeoutMs);
		_talon.configMotionAcceleration(Constants.kAcceleration, Constants.kTimeoutMs);
	...
	public void teleopPeriodic() {
		...
		/* Motion Magic */ 
			...
			/*4096 ticks/rev * numOfRotations Rotations in either direction */
			double numOfRotations = 2;
			double targetPos = 1 * 4096 * numOfRotations;
			double targetPos2 = 1 * 4096 * 4;
			double targetPos3 = 1 * 4096 * 5;
		if (_joy.getRawButton(1)) {
			...
			_talon.set(ControlMode.MotionMagic, targetPos2);
			/* Append more signals to print when in speed mode */
			_sb.append("\terr2:");
			_sb.append(_talon.getClosedLoopError(Constants.kPIDLoopIdx));
			_sb.append("\ttrg2:");
			_sb.append(targetPos2);
			 } 
			 else if(_joy.getRawButton(4)) {
				/* Percent Output */
				//_talon.set(ControlMode.PercentOutput, leftYstick);
				_talon.set(ControlMode.MotionMagic, targetPos3);
				/* Append more signals to print when in speed mode */
				_sb.append("\terr3:");
				_sb.append(_talon.getClosedLoopError(Constants.kPIDLoopIdx));
				_sb.append("\ttrg3:");
				_sb.append(targetPos3);
				 } 
		else {
